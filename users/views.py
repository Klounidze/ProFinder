...
from .forms import UserRegistrationForm, UserLoginForm, UserProfileForm, ProviderEditForm
...
def profile(request):
    ...
    stats = {
        ...
        'providers_count': Provider.objects.filter(created_by=request.user).count()
    }
...
def edit_provider(request, provider_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Информация о специалисте обновлена!')
            return redirect('provider_detail', provider_id=provider.id)
    else:
        form = ProviderEditForm(instance=provider)

    return render(request, 'edit_provider.html', {
        'form': form,
        'provider': provider,
...
def delete_provider(request, provider_id):
    ...
    if request.method == 'POST':
        ...
        provider.delete()
        messages.success(request, f'Специалист "{provider_name}" удален!')
        return redirect('profile')
...
def search_providers(request):
    ...
    data = [{
        ...
        'is_owner': request.user.is_authenticated and p.created_by == request.user
    } for p in providers]
...
def favorites_view(request):
    """Страница с избранными специалистами"""
    favorites = Favorite.objects.filter(user=request.user).select_related('provider')
